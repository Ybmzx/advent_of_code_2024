    ...
    for (auto&& p : antennases) {
        ...
        for (int i = 0; i < vec.size(); i ++) {
            for (int j = 0; j < vec.size(); j ++) {
                if (i == j) continue;
                Point antinode_pos = vec[i], prev_pos = vec[j];
                while (antinode_pos.x <= n && antinode_pos.y <= n &&
                        antinode_pos.x > 0 && antinode_pos.y > 0) {
                    ans += !antinodes[antinode_pos.x][antinode_pos.y];
                    antinodes[antinode_pos.x][antinode_pos.y] = true;
                    Point tmp = get_antinode(antinode_pos, prev_pos);
                    prev_pos = antinode_pos;
                    antinode_pos = tmp;
                }
            }
        }
